 ...
 *           This version uses a linked list which push
 *           and pops numbers into the list and out by deleting it
 *           This version allows the program to be more dynamic
 *
 ...
 *          All error messages are on the console
...
int main() {
    ...
    cout << "Disclaimer: The head of the list starts at the" <<
    " left side unlike the actual stack version of the calculator" << endl << endl;
		...
		if(ev.type == ALLEGRO_EVENT_MOUSE_BUTTON_UP) { //if a mouse button was released on the calculator
            ...
            if (ev.mouse.button & 1){ //if left mouse button was clicked
                ...
                if (ch1[0] != '+' && ch1[0] != '-' &&
                    ...
                    if (Calc.returnTopIndex() <= 19){ //check if list of full
                        expression += ch1[0];
...
char checkClick(int x, int y){ //verifies where the user clicked
